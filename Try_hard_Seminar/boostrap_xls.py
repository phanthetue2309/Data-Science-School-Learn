...
list_all = []
values = [] # tập giá trị thu được dùng để resample

for i in range(0,rows):
    list_row = []
    for j in range(1,11):    # sử dụng đối với file excel # bắt đầu từ 1 vì bản chất trong file excel để chừa 1 ô
...
for i in range(0,n_size):
    train = resample(values, n_samples=len(list_all[0]))    # train là 1 mảng gồm len(list_all[0]) phần tử
...
def standard_division(x, mean_x) : # giá trị trung bình bình phương
    ...
    standard = math.ceil(standard * 100) / 100.0    # làm tròn tới 2 chữ số thập phân
    return standard
...
for list_row in train_all: 
    ...
    list_standard.append(standard_division(list_row, list_mean[-1]))     # standard_division là độ lệch chẩun
