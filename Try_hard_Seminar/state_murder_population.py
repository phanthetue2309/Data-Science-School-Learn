...
def make_table(n):
    ...
    str_arr_part = [''] * n
    str_arr_part[0] = str(int(min_population)) + "-" + str(int(arr_part[0] - 1))
    for i in range(1,n-1):
        str_arr_part[i] = str(int(arr_part[i-1] )) + "-"+ str(int( arr_part[i] - 1 ))
    str_arr_part[n-1] = str(int( arr_part[n-2] )) + "-" + str( int(arr_part[n-2] + part_population )) 
    ...
    plt.bar(left, count, tick_label = left, width = 0.8, color = ['green']) 
