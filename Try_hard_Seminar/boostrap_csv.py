...
count = 0
f = open("boostrap.csv", 'r',encoding="utf-8")  # đọc file

list_all = []

listoflines = f.readlines() # danh sách dòng
i = 0
values = []
for line in listoflines:    # đọc từng dòng
    list_row = []
    if i == 0 : 
        i = 1
        continue
...
def standard_division(x, mean_x) : 
    ...
    standard = math.ceil(standard * 100) / 100.0    # làm tròn 2 số thập phân sau dấu phẩy
    return standard
...
plt.plot(x, probability) # hàm kẻ đường 
plt.xlabel('Bin Number') 
plt.ylabel('Probability') 
plt.title('Mean Boostrap') 
plt.bar(x, probability, color='black', width = 6)  # cột dọc 
plt.show() 
